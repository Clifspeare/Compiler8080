...
char Scanner::getNextChar() { return m_current_file->get(); }

void Scanner::unGetChar(char character) { m_current_file->putback(character); }
...
Token Scanner::getNextToken() {
  ...
  // SYMBOLS / SPECIAL TOKEN TYPE
    ...
    switch (token.value[0]) {  // handle cases where symbol may consist of > 1
                               // character
      case '-':
      ...
      case '%':
        if (c == '=')  // mod assign
          token.value += c;
      case '<':
        if (c == '<') {  // this is one a few special characters that may be in
                         // symbols of 3 characters in size
          token.value += c;
          if ((c = getNextChar()) == '=')  // bit shift left assign
            token.value += c;
          else             // just bit shift left
            unGetChar(c);  // final check for ungetting c won't work for this
                           // 3-character symbol case
        }
...
Scanner::Scanner(char* filepath) { loadSourceFile(filepath); }
