...
std::string getEnumString(TokenType enumval) {
  switch (enumval) {
    ...
    case TokenType::LITERAL:
      return "literal";
    case TokenType::PREPROCESSOR_SYMBOL:
      return "#";
    ...
    default:
      return "token not added in getEnumString";
  }
}
...
void logTokenCreation(const Token& token, const int& depth) {
  ...
  std::cout << getPrintableTokenValue(token) << " add as "
            << getEnumString(token.type) << std::endl;
}
...
std::string getPrintableTokenValue(const Token token) {
  if (token.value == "\n")
    ...
    return "' '";
           ...
           (token.value == "end" || token.value == "eof"))
    return "EOF";
  else
    return token.value;
}